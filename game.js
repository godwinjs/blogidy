...
const wsEndpoint = 'ws://127.0.0.1:9222/devtools/browser/147a7b07-7605-4c83-869f-3f56c736f85b'

async function getAllcircles(){
    
    const connectedBrowser = await puppeteer.connect({ browserWSEndpoint: wsEndpoint });
    const pages = await connectedBrowser.pages();
    const page = pages[1];

    page.click('#start-game')
    // const element = await page.locator('div.light-grid').waitHandle();
    // const circles = await element.evaluate(el => el)
    // // return circles;
    // console.log( circles )
